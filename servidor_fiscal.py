# Archivo: servidor_fiscal_windows.py
# Servidor de impresión para impresoras fiscales The Factory HKA en Windows.
...
def ejecutar_comando_fiscal(comando_str, lineas_esperadas=None):
    ...
    try:
        ...
        if resultado_proceso.stderr:
            raise Exception(f"Error reportado por IntTfhka.exe: {resultado_proceso.stderr.strip()}")
...
def imprimir_factura_fiscal():
    try:
        ...
        with open(ruta_archivo_final, "w", encoding="cp1252") as f:
